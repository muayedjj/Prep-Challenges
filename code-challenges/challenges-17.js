...
const recursionPattern = (int1, int2) => {
    ...
    let output = []
    let arrived = false;
    const recursive = (int1, int2) => {

        output.push(int1);
        if (int1 < 0) arrived = true;
        if (arrived) {return output;}

        recursive(int1 - int2, int2);
        output.push(int1);

        return output;
    }
    return recursive(int1, int2);
}
// -------------------------------------------------------------------------------------------------------
...
const filterLinks = (str) => {
    // write your code here
    let frst = str.indexOf("www.");
    let lstrgx = str.match(/"/g);
    let lst = str.lastIndexOf(lstrgx[0])
    return str.substring(frst, lst);
}
// -------------------------------------------------------------------------------------------------------
...
const isPalindrome = (str) => {
    // write your code here
    let notpalindrome = str.match(/([A-Za-z])\w*/g);
    if (!notpalindrome) return true;
    str = notpalindrome.join('').toLowerCase();

    let ispalindrome = true;
    for (let i = 0; i < str.length / 2; i++) {
        let before = str[i];
        let after = str[str.length - 1 - i];
        ispalindrome = ispalindrome && before === after;
    }
    return ispalindrome;
}
// -------------------------------------------------------------------------------------------------------
